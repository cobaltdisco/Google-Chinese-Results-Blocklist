...
- 将以下 2 个链接，通过点击 Add a subscription 添加到 Subscription 分类下。
  ...
  - [精确匹配](./uBlacklist_subscription.txt?raw=true)：该匹配方式主要是通过 `*://*.xxxx.com/*` 的方式来匹配搜索结果，进行过滤。基本不会有误杀。

  - [模糊匹配](./uBlacklist_match_patterns.txt?raw=true)：该匹配方式主要是通过如 `*://*/list.php?s=*`、`title/小.(百科|知识)网/` 的方式来匹配搜索结果，进行过滤。存在小范围的误杀。
...
- 将[该列表](./GHHbD_perma_ban_list.txt?raw=true)中的网址，通过 List Until -> Import 进行导入。
...
- 开启“附加2-自主拦截域名”，点击 DIY，将[该列表](./GHHbD_perma_ban_list.txt?raw=true)中的网址导入。
